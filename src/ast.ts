...
function walkInternal(node: any, visitor: VisitorFunc, type: NodeType, path: VisitedNode[], index?: number, array?: any[]) {
  ...
  if (node.hasOwnProperty("Node")) {
    ...
    // value list
    node.forEach((value: any, idx: any, values: any) => {
...
export function findValue(item: AstItem, name?: string): AstVal | null {
  if (!name) {
    return item.Val;
  }
...
export function getTokenAtPosition(ast: Ast, pos: AstPosition): AstToken | null {
  let found: AstToken | null = null;
